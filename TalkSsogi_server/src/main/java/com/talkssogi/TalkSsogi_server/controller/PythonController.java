...
public class PythonController {
    ...
    private void uploadFilesToS3(String[] resultLines, int headcount, int crnum) throws IOException {
        ...
        // startIndex와 endIndex가 resultLines의 범위 내에 있는지 확인
        if (startIndex >= resultLines.length) {
            logger.error("시작 인덱스가 범위를 초과했습니다. 시작 인덱스: " + startIndex + ", resultLines 길이: " + resultLines.length);
            throw new IOException("시작 인덱스가 범위를 초과했습니다.");
        }

        if (endIndex > resultLines.length) {
            endIndex = resultLines.length; // endIndex를 resultLines 길이에 맞게 조정
            logger.warn("endIndex가 조정되었습니다. 조정된 endIndex: " + endIndex);
        }
        ...
        for (int i = startIndex; i < endIndex; i++) {
            try {
                ...
                logger.info("업로드할 파일: " + file.getAbsolutePath());
            } catch (ArrayIndexOutOfBoundsException e) {
                logger.error("사용자 파일 경로 추가 중 인덱스 초과 오류 발생: " + e.getMessage());
                throw new IOException("사용자 파일 경로 추가 실패", e);
            }
        }
...
//            Files.deleteIfExists(Paths.get(hourlyFilePath.toString()));
            ...
            if (exitCode == 0) {
                ...
                if (!zeroCountList.isEmpty()) {
                    String[] dates = zeroCountList.get(0).split(", "); // 날짜 목록을 쉼표로 분리
                    int size = dates.length;
                    if (size > 2) {
                        for (int i = 0; i < 2; i++) {
                            zeroCountDatesBuilder.append(dates[i]).append("\n");
                        }
                        zeroCountDatesBuilder.append("외 ").append(size - 2).append("일");
                    } else {
