...
public class PythonController {
    ...
    public ResponseEntity<String> getWordCloudImageUrl(@PathVariable Integer crnum, @PathVariable String userId) {
        try {
            ...
            // file_path 설정
            // S3에서 가져오기
            String filePath = s3DownLoader.getFileUrl(String.format(crnum + (searchWho.equals("group") ? "/group.txt" : "_" + searchWhoIndex + "_personal.txt")));
            ...
            // 분석 결과 파일 경로 추출
            String resultFilePath = crnum + "_" + imgName;  // 결과 파일 경로는 Python 스크립트에서 생성된 파일 경로
            ...
            try (InputStream fileStream = new FileInputStream(resultFile)) {
                String s3Key = "image-files/" + crnum + "_" + resultFile.getName();
    ...
    public ResponseEntity<String> getActivityAnalysisImage(
        ...
        try {
            ...
            // 분석 결과 파일 경로 추출
            String resultFilePath = crnum+"_"+("group".equals(SearchWho) ? SearchWho : searchWhoIndex)+"_"+imgName;  // 결과 파일 경로는 Python 스크립트에서 생성된 파일 경로
            File localResultFile = new File(PYTHON_BASIC_RESULT_FILE_PATH + resultFilePath);

            // S3에 파일 업로드
            try (InputStream fileStream = new FileInputStream(localResultFile)) {
                String key = "images/" + resultFilePath;
            ...
            // S3 URL 생성
            String s3Key = "image-files/" + resultFilePath;
            String resultUrl = s3DownLoader.getFileUrl(s3Key);
