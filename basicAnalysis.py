...
class User:
    def __init__(self, name):
        ...
        self.emoji_count = 0
        self.personal_file_path = f"{name}_personal.txt"
...
class ChatRoom:
    ...
    def save_daily_message_count(self):
        for user_name, user in self.members.items():
            file_path = f"{user_name}_daily_message_count.txt"
    ...
    def save_daily_hourly_message_count(self):
        ...
        for user_name, user in self.members.items():
            file_path = f"{user_name}_daily_hourly_message_count.txt"
            with open(file_path, 'w', encoding='utf-8') as file:
    ...
    def upload_files_to_s3(self, file_paths):
        s3 = boto3.client('s3')
