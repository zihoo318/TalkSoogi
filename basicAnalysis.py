...
class ChatRoom:
    ...
    def preprocess_and_analyze(self):
        if self.version == 'pc':
            self.preprocess_pc_version()
        elif self.version == 'mobile':
            self.preprocess_mobile_version()

        # 채팅방 참여 인원 수를 사용자 이름 목록의 길이로 설정
        self.headcount = str(len(self.members))

    def preprocess_pc_version(self):
        with open(self.file_path, 'r', encoding='utf-8') as file:
            lines = file.readlines()

        date_obj = None
        message_pattern = re.compile(r'\[(.*?)\] \[(오전|오후) (\d{1,2}):(\d{2})\] (.+)')
        date_pattern = re.compile(r'--------------- (\d{4}년 \d{1,2}월 \d{1,2}일) \w요일 ---------------')
        last_user_name = None
        ...
        for line in lines:
            ...
            if match:
                user_name, period, hour, minute, message = match.groups()
                ...
                # date_obj를 이용해 날짜와 시간 객체 생성
                if date_obj is not None:
                    date_time_obj = date_obj.replace(hour=hour, minute=int(minute))
                    date_str = date_time_obj.strftime('%Y-%m-%d')
                    hour_str = date_time_obj.strftime('%H')
    ...
    def preprocess_mobile_version(self):
        with open(self.file_path, 'r', encoding='utf-8') as file:
            lines = file.readlines()
